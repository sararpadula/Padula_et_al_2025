...
library(rstanarm)
mean(MOCH.fe$First.Egg)
var(MOCH.fe$First.Egg)
nrow(MOCH.fe)

fit <- stan_glmer(
  First.Egg ~ Wing_Difference + (1 | Year) + (1 | Year:Group),
  data = MOCH.fe,
  prior_intercept = normal(145, 95, autoscale = TRUE),  
...
library(rstanarm)
mean(BCCH.fe$First.Egg)
var(BCCH.fe$First.Egg)

fit <- stan_lmer(
  First.Egg ~ Wing_Difference + (1 | Year) + (1 | Year:Group),
  data = BCCH.fe,
  prior_intercept = normal(124, 91.5, autoscale = TRUE),  
  prior_aux = cauchy(0,5),       
  prior = normal(3.3, 2),                             
