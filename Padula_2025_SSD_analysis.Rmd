...
SD1 <- lm(Wing.Chord ~ Sex*Species + Year + Bander, data= BSpopI)
SD2 <- lm(Wing.Chord ~ Sex + Species + Year + Bander, data= BSpopI)
SD3 <- lm(Wing.Chord ~ Sex + Species + Year, data= BSpopI)
SD4 <- lm(Wing.Chord ~ Sex + Species + Bander, data= BSpopI)
SDn <- lm(Wing.Chord ~ Species + Year + Bander, data= BSpopI)

anova(SD1, SD2, SD3, SD4, test = "F") # the interaction term is not important but year and bander are

summary(SD2)

anova(SD2, SDn, test = "F")


# Subset BSpop to only include rows where ID matches Male.ID or Female.ID in BS
BSpairs <- BSpop[BSpop$ID %in% c(BS$Male.ID, BS$Female.ID), ]

# View the first few rows of BSpairs
head(BSpairs)
...
BSpairs$Year <- as.factor(BSpairs$Year)
BSpairs$Bander <- as.factor(BSpairs$Bander)

SDp1 <- lm(Wing.Chord ~ Sex*Species + Year + Bander, data= BSpairs)
SDp2 <- lm(Wing.Chord ~ Sex + Species + Year + Bander, data= BSpairs)
SDp3 <- lm(Wing.Chord ~ Sex + Species + Year, data= BSpairs)
SDp4 <- lm(Wing.Chord ~ Sex + Species + Bander, data= BSpairs)
SDpn <- lm(Wing.Chord ~ Species + Year, data= BSpairs)

anova(SDp1, SDp2, SDp3, SDp4, test = "F")

#the interaction term is not important and bander does not make a difference, and year is marginally important

summary(SDp3)

anova(SDp3, SDpn, test = "F")
```
